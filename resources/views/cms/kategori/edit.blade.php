                ...
                <div class="card-body">
                    <form action="{{ route('kategori.update', ['kategori' => $kategori]) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input id="input_category_title" value="{{ old('title', $kategori->title) }}" name="title"
                                type="text" class="form-control form-control-sm @error('title') is-invalid @enderror" />
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <input id="input_category_slug" value="{{ old('slug', $kategori->slug) }}" name="slug"
                                type="text" class="form-control form-control-sm @error('slug') is-invalid @enderror"
                        ...
                        <div class="form-group">
                            ...
                            <select id="select_category_parent" name="parent_category" data-placeholder=""
                                ...
                                @if (old('parent_category', $kategori->parent))
                                    <option value="{{ old('parent_category', $kategori->parent)->id }}" selected>
                                        {{ old('parent_category', $kategori->parent)->title }}
                                    </option>
                                @endif
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="input_category_description" name="desc"
                                class="form-control form-control-sm @error('desc') is-invalid @enderror" rows="3">{{ old('desc', $kategori->desc) }}</textarea>
                            @error('desc')
                                <strong class="text-danger">{{ $message }}</strong>
                            @enderror
                        </div>
